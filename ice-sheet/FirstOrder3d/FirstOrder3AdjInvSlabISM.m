%% FIRSTORDER3ADJINVSLABISM PHGISM-parameter 3-D FO slab-bed inversion.
%
...
% boundary of a periodic sloping slab.  Coordinates are in km, velocities
% are in m/year, and beta is the PHGISM basal friction coefficient.
...
fprintf(['3-D FO ISM slab-bed beta inversion: L = %.2f km, ',...
    'W = %.2f km, H = %.2f km\n'],L,W,H);
...
betaTrue = 1000*(1+0.20*cos(2*pi*xi).*cos(2*pi*eta));
...
for k = 1:maxInverseIt
    ...
    G = assembleparameterderivative(eqn,q,xBeta,yBeta,L,W,pde.beta_scale);
...
xlabel('x'); ylabel('y'); zlabel('\beta');
sgtitle('3-D FO slab-bed beta inversion');
...
sgtitle('recovered top-surface FO ISM fields (m/yr)');
...
clear diaryCleanup;
    ...
    fprintf(2,'\nFirstOrder3AdjInvSlabISM failed. Console output saved to %s\n',...
        diaryFile);
...
function G = assembleparameterderivative(eqn,q,xBeta,yBeta,L,W,betaScale)
    Nm = numel(q);
...
function [u,eqn,info,soln] = solveforward(q,u0,pde,option,...
    ...
    pde.beta = @(pt) pde.beta_scale*periodicRectP1(pt(:,1),pt(:,2),xBeta,yBeta,...
        beta,L,W);
    if isempty(u0)
