%% FULLSTOKES3ADJINVSLABISM 3-D adjoint beta inversion on a slab bed.
%
% This is a PHGISM-internal-unit 3-D slab-bed inverse example.  Coordinates
% are in km, velocity is in m/year, beta is the physical basal coefficient
% in Pa yr m^{-1}, and pressure output is the PHGISM pressure DOF p/1e5.
...
pde.gravity = [0,0,-param.GRAVITY*param.LEN_SCALING^2*...
    param.EQU_SCALING];
pde.g_N = [];
...
q = log(betaInitial(:));
Nm = numel(q);
D = periodicgradient2d(numel(xBeta),numel(yBeta));
...
function param = phgismparameters3d()
...
function printiteration(k,objective,betaLinfRel,gradientNorm,...
    ...
    fprintf(['%3d  %.04e    %.04e    %.04e    %5d   %s  %s  ',...
        '%2d  %8.2f  %s\n'],...
        k,objective,betaLinfRel,gradientNorm,...
