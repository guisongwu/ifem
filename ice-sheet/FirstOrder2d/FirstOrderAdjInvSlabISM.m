%% FIRSTORDERADJINVSLABISM PHGISM-parameter 2-D FO slab-bed inversion.
%
% First-order counterpart of ../FullStokes2d/FullStokesAdjInvSlabISM.m.
% Coordinates are in km, velocities are in m/year, and the inverted beta is
% the PHGISM basal friction coefficient.  Coefficients passed to FirstOrderP2
% use the PHGISM internal equation scaling.
...
% Slab cases.  Set foInvConfig.domainCase to compare how horizontal length
% affects beta identifiability from top-surface velocity data.
domainCase = getconfig(foInvConfig,'domainCase','L4');
switch domainCase
    case 'L1'
        L = 1;
    case 'L2'
    ...
    case 'L4'
        L = 4;
    otherwise
        error('Unknown domainCase "%s". Use L1, L2, or L4.',domainCase);
end
H = 1;
h = 0.1;
slope = tan(0.5*pi/180);

fprintf(['FO ISM slab-bed beta inversion case %s: L = %.2f km, ',...
    'H = %.2f km, h = %.2f km\n'],domainCase,L,H,h);
...
pde.gravity = param.GRAVITY*param.LEN_SCALING^2*param.EQU_SCALING;
pde.f = [];
...
option.eps_reg = getconfig(foInvConfig,'eps_reg',...
    param.MIN_EFFECTIVE_STRAIN*param.LEN_SCALING);
option.maxIt = 200;
option.tol = 1e-6;
...
betaTrueCase = getconfig(foInvConfig,'betaTrueCase','cos1');
switch betaTrueCase
    case 'cos1'
        betaTrue = 1000*(1+0.25*cos(2*pi*xi));
        betaTrueName = 'sin 1 wave';
    case 'sin1'
        betaTrue = 1000*(1+0.25*sin(2*pi*xi));
        betaTrueName = 'sin shifted 1 wave';
    case 'mixed'
        betaTrue = 1000*(1+0.20*cos(2*pi*xi)+0.10*sin(4*pi*xi));
        betaTrueName = 'mixed sin waves';
    otherwise
        error('Unknown betaTrueCase "%s".',betaTrueCase);
end

perturbationCase = getconfig(foInvConfig,'perturbationCase','sin1');
switch perturbationCase
    case 'sin1'
        betaPerturbation = 0.20*sin(2*pi*xi)+0.05;
        perturbationName = 'sin 1 wave';
    case 'sin2'
        betaPerturbation = 0.12*sin(4*pi*xi)+0.05;
        perturbationName = 'sin 2 waves';
    case 'sin1cos2'
        betaPerturbation = 0.15*sin(2*pi*xi)+...
            0.08*cos(4*pi*xi)+0.05;
        perturbationName = 'mixed sin waves';
    otherwise
        error('Unknown perturbationCase "%s".',perturbationCase);
end
...
[uTrue,eqnTrue,trueInfo,node,elem,bdFlag] = solveforward(...
    qTrue,[],pde,option,xBeta,L);
assert(trueInfo.converged,'The truth solve did not converge.');

dofNode = eqnTrue.dofNode;
surfaceLevel = H-slope*dofNode(:,1);
...
%% Inverse options
maxInverseIt = getconfig(foInvConfig,'maxInverseIt',30);
alpha = getconfig(foInvConfig,'alpha',0);
lambda = getconfig(foInvConfig,'lambda',1e-7);
pcgTolerance = 1e-8;
pcgMaxIt = 50;
stepTolerance = 1e-7;
...
for k = 1:maxInverseIt
    iterationTimer = tic;
    [u,eqn,forwardInfo,node,elem,bdFlag] = solveforward(...
        q,uWarm,pde,option,xBeta,L);
    ...
    if objective < objectiveTolerance
        ...
        printiterationrow(k,objective,history.parameterErrorRelativeLinf(k),...
            norm(gradient),forwardInfo.itStep,NaN,NaN,0,'obj',...
            iterationTime);
    ...
    if norm(gradient) <= gradientTolerance
        iterationTime = toc(iterationTimer);
        printiterationrow(k,objective,history.parameterErrorRelativeLinf(k),...
            norm(gradient),forwardInfo.itStep,NaN,NaN,0,'grad',...
            iterationTime);
        history = trimhistory(history,k);
        break
    end
...
title(sprintf('FO ISM slab-bed beta inversion: true %s, perturbation %s',...
...
subplot(1,2,1);
showmesh(node,elem);
title('FO ISM slab mesh','FontSize',14);
...
function [u,eqn,info,node,elem,bdFlag] = solveforward(...
    ...
    beta = exp(q(:));
    option.beta = @(pt) pde.beta_scale*periodicP1(pt(:,1),xBeta,beta,L);
    option.A = pde.A;
    option.n = pde.n;
    option.m = pde.m;
    option.rho = pde.rho;
...
function printiterationrow(k,objective,betaLinfRel,...
        ...
        lineSearchCount,stopReason,iterationTime)
    width = [3,12,11,12,7,5,10,2,10,4];
