...
本文整理 `FirstOrderP2.m` 和 `FirstOrderAdjInvSlab.m` 中使用的
...
使用 P2 三角元离散水平速度，`FirstOrderAdjInvSlab.m` 用顶部水平速度观测反演底部
摩擦系数。
...
`FirstOrderAdjInvSlab.m` 使用离散归一化版本：
...
`FirstOrderAdjInvSlab.m` 用周期 P1 函数表示 $q$ 和 $\beta$：
