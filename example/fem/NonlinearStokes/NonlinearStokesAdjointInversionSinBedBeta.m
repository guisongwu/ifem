%% NONLINEARSTOKESADJOINTINVERSIONSINBEDBETA Invert beta on a sinusoidal bed.
%
...
fprintf('Sin-bed beta inversion case: L = %.04e, H = %.04e, Nx = %d, Nz = %d\n',...
    L,H,Nx,Nz);
...
function printiterationheader()
    fprintf('\n');
    fprintf([' it    objective  betaLinfAbs betaLinfRel       |grad| ',...
             'fPicard pcgIt     pcgRel ls stop\n']);
    fprintf(['--- ------------ ------------ ----------- ------------ ',...
             '------- ----- ---------- -- ----\n']);
end
...
function printiterationrow(k,objective,betaLinfAbs,betaLinfRel,...
    ...
    fprintf(['%3d %12.04e %12.04e %11.04e %12.04e ',...
             '%7d %s %s %2d %s\n'],...
        k,objective,betaLinfAbs,betaLinfRel,gradientNorm,...
        forwardPicard,pcgItText,pcgRelText,...
        lineSearchCount,stopReason);
end
