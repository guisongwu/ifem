...
“一致”两个字的意思是：这个矩阵必须和当前使用的非线性残差 $R(U,q)$ 完全匹配。残差里有哪些非线性项，切线矩阵里就要包含这些项对未知量的导数。
...
2. 底部 Weertman 滑移牵引依赖速度：

   $$
   \beta |Tu|^{m-1}Tu.
   $$

因此，一致切线矩阵不只是普通 Stokes 刚度矩阵，还必须包括：

- 黏性项中 $\eta(u)$ 对 $u$ 的导数；
- 底部滑移项中 $|Tu|^{m-1}Tu$ 对 $u$ 的导数；
- 压力-速度耦合块；
- 不可压约束块；
- 周期边界、底部法向约束等约束块。

如果把这些导数项漏掉，得到的矩阵就不是当前非线性残差的一致线性化。

### 10.1 和 Picard 矩阵的区别

Picard 迭代中，每一步通常把黏度和滑移系数冻结在上一轮速度上，然后解一个线性 Stokes 型问题。这个线性系统的矩阵可以称为 Picard 矩阵。

它大致对应：

```text
把 eta(u) 看成已知系数后得到的线性矩阵。
```

但一致切线矩阵对应的是：

```text
对完整非线性残差 R(U,q) 做精确一阶微分得到的 Jacobian。
```
...
```text
Picard 矩阵:
  冻结 eta(u) 和滑移系数，只用于稳健求正问题。

一致切线矩阵:
  包含 eta(u)、滑移律、约束等所有残差项的一阶导数，
  用于灵敏度、伴随梯度和 Gauss--Newton 反演。
```

所以 Picard 矩阵可以让正问题收敛，但它通常不能直接用于精确伴随梯度。
...
如果用某种迭代方法把 $u^3$ 近似成 $a u$，那么每一步会解

$$
a u=f.
$$

这里的 $a$ 可以帮助迭代求解，但它不一定等于真实导数 $3u^2$。如果要做灵敏度分析或 Newton 线性化，就必须使用 $R'(u)=3u^2$，而不是随便使用迭代中冻结出来的系数 $a$。

非线性 Stokes 中的一致切线矩阵就是这个思想的矩阵版本。
