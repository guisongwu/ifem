...
function D = rectangulargradient2d(nx,ny)
    xEdges = max(nx-1,0)*ny;
    yEdges = nx*max(ny-1,0);
    rows = zeros(2*(xEdges+yEdges),1);
    cols = zeros(2*(xEdges+yEdges),1);
    vals = zeros(2*(xEdges+yEdges),1);
    cursor = 0;
    row = 0;
    for iy = 1:ny
        for ix = 1:nx-1
    ...
    D = sparse(rows,cols,vals,row,nx*ny);
end
