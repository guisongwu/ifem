...
for level = 1:nlevel
    ...
    node = perturbinteriornode(node,h);
    node(:,1) = L*node(:,1);
    node(:,2) = W*node(:,2);
    zeta = node(:,3);
...
rateU = [NaN;log(errU(1:end-1)./errU(2:end))./...
    log(hlist(1:end-1)./hlist(2:end))];
rateP = [NaN;log(errP(1:end-1)./errP(2:end))./...
    log(hlist(1:end-1)./hlist(2:end))];
...
function node = perturbinteriornode(node,h)
xi = node(:,1);
yi = node(:,2);
zeta = node(:,3);
amp = 0.06*h;
shape = sin(pi*xi).*sin(pi*yi).*sin(pi*zeta);
node(:,1) = xi+amp*shape.*sin(2*pi*yi).*sin(pi*zeta);
node(:,2) = yi+amp*shape.*sin(2*pi*zeta).*sin(pi*xi);
node(:,3) = zeta+amp*shape.*sin(2*pi*xi).*sin(pi*yi);
end
