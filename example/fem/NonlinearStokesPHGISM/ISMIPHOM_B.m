...
pismFigureData = readpismfiguredata('ISMIPHOM_B_pism_fig29_extracted.csv');
officialFSCurveData = readofficialfscurvedata(...
    'ISMIPHOM_B_official_fs_curve.csv');
...
result = table('Size',[numel(lengthList),10],...
    ...
    'VariableNames',{'lengthKm','converged','PicardSteps',...
                     ...
                     'officialFSMean','officialFSMax','pismFig29Mean',...
                     'pismFig29Max','maxDifference'});
...
for iLength = 1:numel(lengthList)
    ...
    if ~isempty(pismFigureProfile{iLength})
        result.pismFig29Mean(iLength) = mean(pismFigureProfile{iLength}(:,2));
        result.pismFig29Max(iLength) = max(pismFigureProfile{iLength}(:,2));
    else
        result.pismFig29Mean(iLength) = NaN;
        result.pismFig29Max(iLength) = NaN;
    end
...
for iLength = 1:numel(lengthList)
    ...
    if ~isempty(pismFigureProfile{iLength})
        ...
        plot(pismProfile(:,1),pismProfile(:,2),'m--','LineWidth',1.5,...
            'DisplayName','PISM Fig. 29 extracted');
    end
