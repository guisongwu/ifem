...
pismFigureData = readpismfiguredata('ISMIPHOM_D_pism_fig31_extracted.csv');
officialFSCurveData = readofficialfscurvedata(...
    'ISMIPHOM_D_official_fs_curve.csv');
...
result = table('Size',[numel(lengthList),10],...
    ...
    'VariableNames',{'lengthKm','converged','PicardSteps',...
                     ...
                     'officialFSMean','officialFSMax','pismFig31Mean',...
                     'pismFig31Max','maxDifference'});
surfaceProfile = cell(numel(lengthList),1);
pismFigureProfile = cell(numel(lengthList),1);
...
for iLength = 1:numel(lengthList)
    ...
    pismFigureProfile{iLength} = pismFigureData{iLength};
    if ~isempty(pismFigureProfile{iLength})
        result.pismFig31Mean(iLength) = mean(pismFigureProfile{iLength}(:,2));
        result.pismFig31Max(iLength) = max(pismFigureProfile{iLength}(:,2));
    else
        result.pismFig31Mean(iLength) = NaN;
        result.pismFig31Max(iLength) = NaN;
    end
...
for iLength = 1:numel(lengthList)
    ...
    hold on;
    if ~isempty(pismFigureProfile{iLength})
        pismProfile = pismFigureProfile{iLength};
        plot(pismProfile(:,1),pismProfile(:,2),'m--','LineWidth',1.5,...
            'DisplayName','PISM Fig. 31 extracted');
    end
...
function pismFigureData = readpismfiguredata(filename)
    lengthKm = [5;10;20;40;80;160];
    pismFigureData = cell(numel(lengthKm),1);
    if ~isfile(filename)
        return;
    end
    ...
    for iLength = 1:numel(lengthKm)
        isCurrentLength = data.lengthKm == lengthKm(iLength);
        pismFigureData{iLength} = [data.xOverL(isCurrentLength),...
            data.uxPISM(isCurrentLength)];
    end
end
