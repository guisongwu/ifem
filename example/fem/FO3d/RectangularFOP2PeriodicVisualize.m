function figs = RectangularFOP2PeriodicVisualize(option,soln,eqn,info,node,elem,bdFlag)
%% RECTANGULARFOP2PERIODICVISUALIZE visualize the periodic box FO solution.
%
% Example:
%
%   RectangularFOP2PeriodicVisualize
%
% Reuse an existing solve:
%
%   [soln,eqn,info,node,elem,bdFlag] = RectangularFOP2Periodic(option);
%   RectangularFOP2PeriodicVisualize(option,soln,eqn,info,node,elem,bdFlag)
%
% To save figures:
%
%   option.exportPng = true;
%   option.outputDir = 'fig';
%   RectangularFOP2PeriodicVisualize(option)
...
if nargin < 2 || isempty(soln)
    ...
    [soln,eqn,info,node,elem,bdFlag] = RectangularFOP2Periodic(solveOption);
elseif nargin < 7
...
exportfigure(figs(1),outputDir,'RectangularFOP2Periodic-boundary-speed.png',exportPng);
...
quiver3(xs(1:stride:end,1:stride:end),...
        ...
        'k');
hold off;
exportfigure(figs(2),outputDir,'RectangularFOP2Periodic-mid-slice-speed.png',exportPng);
...
exportfigure(figs(3),outputDir,'RectangularFOP2Periodic-bed-speed.png',exportPng);
