function [soln,eqn,info] = Poisson_periodic(node,elem,bdFlag,pde,option)
%% POISSON Poisson equation: P1 linear element
% 
%  Note:
%  Hard modified to periodic, u_{x=0} = u_{x=1}.
%  Dirich only.
% 
% 
% 
...
if periodic
    ...
    % not true for slab test
    assert(norm(dof_y(I0) - (dof_y(I1) + slope*(dof_x(I1))) ) < 1e-12);
    ...
    Np = length(I1);
    Bp = speye(N - Np, N);
    Bp(I0,I1) = speye(Np, Np);
    
    clear I0 I1 dof_x dof_y
end
...
if periodic
    ...
    [i,j,s] = find(AD);
    Ap = sparse(Ip(i), Ip(j), s, N-Np, N-Np);
...
if nargout == 1
    ...
    if periodic
        eqn = struct('A',AD,'b',b, 'Ap', Ap, 'bp', bp, 'freeNode',freeNode,'Lap',A, 'Ip', Ip);
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~isempty(Robin) % && ~isempty(pde.g_R)
        ...
        if option.verb >= 1
            fprintf('Robin: Modify mat\n');
        end

        % ibot = min([Robin(:,1), Robin(:,2)], [], 2);
        % ibot = (ibot - 1) / 11 + 1;
        % fprintf('ibot %d\n', ibot);
        ...
        assert(~isempty(pde.g_R));
        if isnumeric(pde.g_R)
            if option.verb >= 1
                warning('numerical g_R');
            end
            assert(iscolumn(pde.g_R));
            beta = pde.g_R;
            beta_shift = [beta(2:end); beta(1)];
            % beta_mid = .5 * (beta + beta_shift);
            beta_elem = [beta beta_shift];
        else
            beta_mid = pde.g_R(mid);
        end
        ...
        if 0
            ...
            temp = beta_mid.*edgeLength;
            ss = [1/3*temp, 1/6*temp, 1/6*temp, 1/3*temp];
            A = A + sparse(ii,jj,ss,Ndof,Ndof);
        else
            [lambdagR,weightgR] = quadpts1(option.gNquadorder);
            phigR = lambdagR;                 % linear bases
            ...
            if isnumeric(pde.g_R)
                for pp = 1:nQuadgR
                    ...
                    gRp = lambdagR(pp,1)*beta_elem(:,1) ...
                           + lambdagR(pp,2)*beta_elem(:,2);
    ...
    if option.verb >= 1
        fprintf('fixed: %d, free: %d\n', length(fixedNode), length(freeNode));
        %fixedNode
    end
...
%     end
    ...
    if ~isempty(Neumann) 
        ...
        assert(~isempty(pde.g_N));
        
        if isnumeric(pde.g_N)
            if option.verb >= 1
                warning('numerical g_N');
            end
            assert(iscolumn(pde.g_N));
            gNp = pde.g_N;
            gNp_shift = [gNp(2:end); gNp(1)];
            %gNp_mid = .5 * (gNp + gNp_shift);
            gNp_elem = [gNp gNp_shift];
        end
        ...
        if isnumeric(pde.g_N)
            for pp = 1:nQuadgN
                ...
                gNp = lambdagN(pp,1) * gNp_elem(:,1) ...
                       + lambdagN(pp,2) * gNp_elem(:,2);
    ...
    if ~isempty(Robin) 
        ...
        assert(~isempty(pde.g_RN));
        
        if isnumeric(pde.g_RN)
            if option.verb >= 1
                warning('numerical g_RN');
            end
            assert(iscolumn(pde.g_RN));
            gNp = pde.g_RN;
            gNp_shift = [gNp(2:end); gNp(1)];
            %gNp_mid = .5 * (gNp + gNp_shift);
            gNp_elem = [gNp gNp_shift];
        ...
        if isnumeric(pde.g_RN)
            ...
            for pp = 1:nQuadgN
                ...
                gNp = pde.g_RN(ppxy);    % function evaluation
        ...
        ge = ge.*repmat(el,1,2);
        b = b + accumarray(Robin(:), ge(:),[Ndof,1]); 
    end
    ...
    AD(fixedNode, :) = 0;
    AD(fixedNode, fixedNode) = speye(length(fixedNode), length(fixedNode));
