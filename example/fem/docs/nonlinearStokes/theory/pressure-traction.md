...
这里的“相容”可以理解为：牵引边界已经选定了某一个压力代表，而压力零均值又试图选定另一个代表。只有当这两个代表恰好相同，也就是牵引边界选出的解本来就满足

$$
\int_\Omega p\,\mathrm{d}x=0,
$$

时，两个条件才能同时成立。否则，强制零均值并不是单纯改变压力规范，而是在改变原来的边值问题。

更具体地说，若自然牵引问题给出的压力平均值为
...
但这会把同一个速度场对应的牵引从 $g_N$ 改成

$$
g_{N,0}=g_N+\overline p\,n.
$$

所以“牵引 $g_N$ + 压力零均值”和“牵引
$g_N+\overline p n$ + 压力零均值”是两个不同的边值问题。第二个写法才是原始牵引问题在零均值压力规范下的等价表示；第一个写法通常会改变原问题。
...
同一件事也可以从常数压力测试模式看出来。若压力空间中包含常数函数，连续不可压缩约束要求
...
如果所有速度边界都给定且满足总通量为零，这个条件是速度边界数据的相容性条件，压力常数仍未固定。相反，如果边界上存在牵引段，测试函数在该边界的法向分量不被固定，常数压力模式会通过边界项与法向牵引配对。因此牵引数据会参与决定压力常数。

这也是为什么“压力零均值”在纯速度边界问题中是规范选择，而在含牵引边界问题中可能变成额外约束：前者没有绝对牵引来测量压力常数，后者已经有了。
...
这个结论可以从增广系统的第二行直接读出。原始离散不可压缩方程是

$$
Bu=0.
$$

加入压力均值约束后，为了同时满足动量方程、压力均值和原有牵引，系统引入了乘子 $\lambda_p$。第二行变为

$$
Bu=-m\lambda_p.
$$

如果 $\lambda_p\ne0$，则速度散度不再在离散压力测试空间中为零，而是等于一个常数压力模式对应的右端。由于 $m$ 是压力质量向量，这个右端在物理空间中表现为近似常数的散度误差。

因此，看到

$$
\nabla\cdot u_h \approx \text{常数}
$$

并且该常数不随网格加密下降时，首先应该检查压力规范和牵引数据是否相容，而不是把它解释为普通有限元收敛问题。
...
也就是说，牵引自由边界不是“没有牵引条件”，而是“总牵引被指定为零”。它同样会固定压力常数。
