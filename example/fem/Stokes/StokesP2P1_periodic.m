    ...
    % g = zeros(Np,1);
        ...
        if ~isempty(bdFlag)
            ...
            if use_slip
                ...
                if isnumeric(pde.g_Dn)
                    if ~periodic
                        rot_info.Un = pde.g_Dn([1:nRobin, 1, nRobin+1:2*nRobin]);
                    else
                        rot_info.Un = pde.g_Dn([1:nRobin, nRobin+1:2*nRobin]);
                    end
                else
                        rot_info.Un = pde.g_Dn([node(IR,:); 
        ...
        if ~isempty(Robin) 
            ...
            for pp = 1:nQuad
                ...
                if isnumeric(pde.g_R)
                    assert(size(pde.g_R, 1) == 2 * nRobin);
                    
                    gr1 = pde.g_R([1:nRobin], :);
                    gr2 = pde.g_R([2:nRobin, 1], :);
                    gr3 = pde.g_R([nRobin+1:2*nRobin], :);
                    gr = gr1 .*  bdphi(pp,1) ...
                         + gr2 .*  bdphi(pp,2) ...
                         + gr3 .*  bdphi(pp,3);
                    % xqt = [xqt pxy(:,1)];
                    ...
                    % if use_linear
                    %     gr = [pde.g_R([1:nRobin]), pde.g_R([2:nRobin, 1]), .5*(pde.g_R([1:nRobin]) + pde.g_R([2:nRobin, 1]))];
                    % else

                    % gr = [pde.g_R([1:nRobin]), pde.g_R([2:nRobin, 1]), pde.g_R([nRobin+1:2*nRobin])];

                    %end

                    gp1 = pde.g_RN([1:nRobin], :);
                    gp2 = pde.g_RN([2:nRobin, 1], :);
                    gp3 = pde.g_RN([nRobin+1:2*nRobin], :);
                    
                    gp = gp1 .*  bdphi(pp,1) ...
                         + gp2 .*  bdphi(pp,2) ...
                         + gp3 .*  bdphi(pp,3);

                    if size(pde.g_RN, 2) == 2
                        ;
                    elseif size(pde.g_RN, 2) == 4
                        % g_RN = f1 * f2
                        gp(:, 1) = gp(:, 1).*gp(:, 2);
                        gp(:, 2) = gp(:, 3).*gp(:, 4);
                        gp = gp(:, [1 2]);
                    else
                        error('g_RN dim not right')
                    end
