function pde = stokes_data_P2_slab
%% STOKESDATA2 data for Stokes equations
%
% The solution u is a polynomial satisfying the zero Dirichlet boundary
% condition. 
%
% Created by Ming Wang.
%
    pde = struct('f', @f, ...
                 'fp', @fp, ...
                 'exactp', @exactp, ...
                 'exactu',@exactu, ...
                 'exactux',@exactux, ...
                 'exactuy',@exactuy, ...
                 'g_D', @exactu, 'g_N', @g_N, 'g_R',@g_R, 'g_RN', @g_RN, 'g_Dn', @g_Dn);
%

global slope;

% x*(-(slope*x) + y)
% -0.5*power(-(slope*x) + y,2)
% x + slope*x - y
% 1 + 3*slope
% power(slope,2)
% -(slope*x)

%% subfunction
    function z = exactux(pt)
        x = pt(:,1); y = pt(:,2);
        z = x.*(-(slope*x) + y);
    end
    function z = exactuy(pt)
        x = pt(:,1); y = pt(:,2);
        z = -0.5*power(-(slope*x) + y,2);
    end

    function z = exactu(pt)
        x = pt(:,1); y = pt(:,2);
        z(:,1) = exactux(pt);
        z(:,2) = exactuy(pt);
    end

    function z = exactp(pt)
        x = pt(:,1); y = pt(:,2);
        z = x + slope*x - y;
    end

    function z = f(pt) % load data (right hand side function)
        x = pt(:,1); y = pt(:,2);
        z(:,1) = x*0 + 1 + 3*slope;
        z(:,2) = x*0 + power(slope,2);
    end

    function z = fp(pt) % load data (right hand side function)
        x = pt(:,1); y = pt(:,2);
        z = -(slope*x);
    end



    % -2*slope*x + y,
    % x,
    % slope*(-(slope*x) + y),
    % slope*x - y,
    % 1 + slope,
    % -1    

    % Derivative of the exact solution
    function du =  Du(pt)
        x = pt(:,1); y = pt(:,2);
        du(:,1,1) = x*0 -2*slope*x + y;
        du(:,1,2) = x*0 + x;
        du(:,2,1) = x*0 + slope*(-(slope*x) + y);
        du(:,2,2) = x*0 + slope*x - y;
    end

    function dp =  Dp(pt)
        x = pt(:,1); y = pt(:,2);
        dp(:,1) = x*0 + 1 + slope;
        dp(:,2) = x*0 + -1; 
    end




    % g_N = dudn
    function uN = g_N(pt)
        x = pt(:,1); y = pt(:,2);
        y1 = y + slope * x;
        du = Du(pt);
        p = exactp(pt);

        n = [slope, 1];
        n = n / norm(n);

        % size(du)
        % size(p)
        
        uN(:,1) = n(1)*du(:,1,1) + n(2)*du(:,1,2) - n(1) * p(:);
        uN(:,2) = n(1)*du(:,2,1) + n(2)*du(:,2,2) - n(2) * p(:);
    end



    % g_RN = dudn + beta u
    function uR = g_R(pt)
        x = pt(:,1); y = pt(:,2);
        uR = 0*x + 1.;
    end
    
    function uN = g_RN(pt)
        x = pt(:,1); y = pt(:,2);
        y1 = y + slope * x;
        du = Du(pt);

        u = exactu(pt);
        p = exactp(pt);
        du = Du(pt);

        n = [-slope, -1];
        n = n / norm(n);

        beta = g_R(pt);

        uN(:,1) = n(1)*du(:,1,1) + n(2)*du(:,1,2) - n(1) * p(:) + beta .* u(:, 1);
        uN(:,2) = n(1)*du(:,2,1) + n(2)*du(:,2,2) - n(2) * p(:) + beta .* u(:, 2);
    end

    function uN = g_Dn(pt)
        x = pt(:,1); y = pt(:,2);
        y1 = y + slope * x;

        u = exactu(pt);
        n = [-slope, -1];
        n = n / norm(n);

        uN = u(:,1) * n(1) + u(:,2) * n(2);
    end
    
end
